...
public class ClassFileModelLoader {
	...
	private ConcreteClassifier constructClassifier(JavaClass clazz) {
		...
		if (clazz.isEnum()) { // Check first because enum is also a class.
		...
		if (clazz.isClass() && !clazz.isEnum()) {
			org.emftext.language.java.classifiers.Class emfClass = (Class) emfClassifier;
			if (clazz.getSuperclassName() != null) {
				emfClass.setExtends(createReferenceToClassifier(clazz.getSuperclassName()));
				typeArgumentSig = constructTypeArguments(typeArgumentSig, (ClassifierReference) emfClass.getExtends(),
					null, emfClassifier);
			}
		} else {
			// Cut away the Object signature which is also there for interfaces.
	...
	private Member constructMethod(org.apache.bcel.classfile.Method method,
			ConcreteClassifier emfClassifier, boolean withVariableLength) {
		Method emfMethod = null;
		if (emfClassifier instanceof Annotation || emfClassifier instanceof Interface) {
		...
		for (int i = 0; i < method.getArgumentTypes().length; i++) {
			...
			if (i == method.getArgumentTypes().length - 1 && withVariableLength) {
				...
				emfMethod.getParameters().add(constructParameter(argType, paramName));
			}
		}

		EList<TypeReference> tpList = constructMethodTypeParameterReferences(plainSignature,
				(Method) emfMethod, emfClassifier);
		for (int i = 0; i < tpList.size(); i++) {
			...
			if (typeParameterReference != null) {
				...
				if (typeReference instanceof ClassifierReference) {
					// Replace with parameter if there is one.
		...
		if (!"".equals(plainSignature) && !(emfClassifier instanceof Enumeration)) {
			...
			for (Parameter parameter : emfMethod.getParameters()) {
				...
				if (parameterTypeRef instanceof ClassifierReference) {
					argumentSignature = constructTypeArguments(argumentSignature,
							(ClassifierReference) parameterTypeRef, emfMethod, emfClassifier);
				} else {
					argumentSignature = constructTypeArguments(argumentSignature,
							null, emfMethod, emfClassifier);
				}
			}
		}
	...
	private Parameter constructParameter(org.apache.bcel.generic.Type attrType, String paramName) {
	...
	private Parameter constructVariableLengthParameter(org.apache.bcel.generic.Type attrType, String paramName) {
	...
	private void constructModifiers(AnnotableAndModifiable emfMember, org.apache.bcel.classfile.AccessFlags member) {
	...
	private ClassifierReference constructTypeParameterReference(String name,
			TypeParametrizable method, ConcreteClassifier emfClassifier) {
	...
	private ClassifierReference constructReturnTypeParameterReference(String signature,
			TypedElement element, ConcreteClassifier emfClassifier) {
	...
	private EList<TypeReference> constructMethodTypeParameterReferences(String signature,
			Method method, ConcreteClassifier emfClassifier) {
		...
		// Cut away all inner type arguments.
	...
	private String constructTypeArguments(String s, ClassifierReference typeRef,
			TypeParametrizable method, ConcreteClassifier emfClassifier) {
		if ("".equals(s) || s == null) {
			return "";
		}
						...
						if ((internalNext == ';') && internalBracketCount == 0) {
							...
							if (charArray[internalBegin] == '*' || charArray[internalBegin] == '?') {
								...
								if (charArray[internalBegin] == 'T') {
									argumentType = constructTypeParameterReference(fullName, method, emfClassifier);
								} else {
	...
	private TypeReference createReferenceToClassifier(String fullClassifierName) {
		...
		Classifier classifier = (Classifier) JavaClasspath.get().getConcreteClassifier(fullClassifierName);
		ClassifierReference classifierReference = TypesFactory.eINSTANCE.createClassifierReference();
	...
	private List<String> extractParameterNames(final org.apache.bcel.classfile.Method method) {
